...
impl<T: StoreType, U: ApiClient, V: SamProtocolClient> Client<T, U, V> {
    ...
    async fn delete_device(self) {
        todo!()
    }

    /// Get users account id from username
    async fn get_user_account_id(&self, _username: &str) -> Result<AccountId, ClientError> {
        todo!()
    }
    ...
    async fn subscribe(&mut self) -> Result<Receiver<DecryptedEnvelope>, ClientError> {
        todo!()
    }
    ...
    async fn create_provision(&mut self) -> Result<LinkDeviceToken, ClientError> {
        todo!()
    }
}
