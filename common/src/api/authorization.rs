...
impl BasicAuthorizationHeader {
    ...
    pub fn decode(s: &str) -> Result<BasicAuthorizationHeader, LibError> {
        if !s.starts_with("Basic") {
            return Err(LibError::AuthorizationError(
                "Invalid authorization header type.".to_string(),
            ));
        }
        let space_index = s.find(" ").ok_or_else(|| {
            LibError::AuthorizationError("No ' ' character in AuthorizationHeader.".to_string())
        })?;
        let s = String::from_utf8(
            BASE64_STANDARD
                .decode(&s[space_index + 1..])
                .map_err(|err| LibError::AuthorizationError(err.to_string()))?,
        )
        .map_err(|err| LibError::AuthorizationError(err.to_string()))?;

        let colon_index = s.find(":").ok_or_else(|| {
            LibError::AuthorizationError("No ':' character in AuthorizationHeader.".to_string())
        })?;
        let dot_index = s.find(".").ok_or_else(|| {
            LibError::AuthorizationError("No '.' character in AuthorizationHeader.".to_string())
        })?;
        ...
        let device_id = device_part
            ...
            .parse()
            .map_err(|err: ParseIntError| LibError::AuthorizationError(err.to_string()))?;
